...
import { useERC20TokenAllowance, useERC1155ApprovalForAll } from './useContractCall';
...
export function useGetSafesAndGroupsManagedByUser(account) {
  ...
  const { result: rGroupsByManagers, reexecuteQuery: reexecuteGroupsByManagers } = useQueryGroupsByManagers(
  ...
  useEffect(() => {
    if (rGroupsByManagers.data) {
      setGroupsIds(rGroupsByManagers.data.groups.map((group) => group.id));
    }
  }, [rGroupsByManagers]);
