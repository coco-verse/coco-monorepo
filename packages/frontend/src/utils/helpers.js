...
import { MULTIPLIER_BASE, MULTIPLIER } from '.';
...
export function getDecStrAvgPriceBN(amountIn, amountOut) {
  ...
  return formatBNToDecimal(amountIn.mul(MULTIPLIER).div(amountOut), MULTIPLIER_BASE, true, 6);
}
...
export function postSignTypedDataV4Helper(groupAddress, marketIdentifier, amount1Bn, chainId) {
  const domain = [
