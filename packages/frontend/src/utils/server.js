...
export const baseInstance = axios.create({
  baseURL: (() => {
    if (process.env.REACT_APP_VERCEL_ENV == 'DEVELOPMENT') {
      ...
      return 'https://reddit.main.cocoverse.club/api';
...
export async function initialiseSubmission(marketIdentifier, challengeData, challengeDataSignature, groupAddress) {
  try {
    const { data } = await baseInstance.request({
